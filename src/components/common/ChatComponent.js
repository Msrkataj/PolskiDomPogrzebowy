...
import { doc, getDoc, addDoc, onSnapshot, collection, updateDoc } from 'firebase/firestore';
...
const Chat = ({ minimized, setMinimized }) => {
    ...
    const handleConsultantClick = async () => {
        ...
        try {
            // Pobierz formData z Firestore na podstawie formId z localStorage
            const formId = localStorage.getItem('formId') || localStorage.getItem('userId');
            let formData = null;
            ...
            // Tworzenie nowego czatu w Firestore z dołączonym formData
            ...
            // Nasłuchiwanie na zmiany w czacie
            ...
            console.error("Błąd podczas łączenia z doradcą:", error);
            setIsConnecting(false);
        }
    };
    ...
    const handleCloseChat = async () => {
        ...
        const chatRef = doc(db, 'chat', chatId);
        await updateDoc(chatRef, { active: false, closed: true, closedWho: 'Klient' });
                ...
                <div className={`chat-container ${isConnecting ? 'connecting' : ''}`}>
                    <div className="chat-header">
                        ...
                        <div className="header-actions-chat">
                            ...
                            {connected && (
                                <button className="back-button-chat" onClick={handleCloseChat}>Zakończ <br/> konwersację
                                </button>
                            )}
                            {!chatClosed && isConnecting && (
                                <button className="back-button-chat" onClick={handleCloseChat}>Zamknij czat</button>
                            )}
                        </div>
